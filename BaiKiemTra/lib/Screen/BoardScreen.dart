...
class _BoardScreenState extends State<BoardScreen> {
          ...
          body: Stack(
            ...
            children: [
              TabBarView(
                children: <Widget>[
                  Container(
                    child: (listTask.length == 0
                        ? ScreenNoTask()
                        : ScreenHasTask(listTask)),
                  ),
                  Container(
                    child: (listTaskUnCompleted.length == 0
                        ? ScreenNoTask()
                        : ScreenHasTask(listTaskUnCompleted)),
                  ),
                  Container(
                    child: (listTaskCompleted.length == 0
                        ? ScreenNoTask()
                        : ScreenHasTask(listTaskCompleted)),
                  ),
                  Container(
                    child: (listTaskFavourited.length == 0
                        ? ScreenNoTask()
                        : ScreenHasTask(listTaskFavourited)),
                  )
                ],
              ),
              Container(
  ...
  TaskGui(TaskInfo taskInfo) {
    return Container(
      ...
      decoration: BoxDecoration(
          color: taskInfo.color, borderRadius: BorderRadius.circular(10)),
      child: ListTile(
        leading: Checkbox(
